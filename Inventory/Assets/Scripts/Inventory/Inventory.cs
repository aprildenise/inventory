...
public class Inventory : MonoBehaviour
{
    ...
    /// <summary>
    /// Called when the player hits the left navigation button. Goes to the next page
    /// that is to the left of the currentPage in the list of InventoryPages.
    /// Also, show the right navigation button as needed.
    /// </summary>
    ...
    /// <summary>
    /// Hide the inventory menu by setting it's canvas as false. However,
    /// if the player is currently interacting with an item in the menu,
    /// prevent the menu from being hidden until the player is done interacting
    /// with the item.
    /// </summary>
    public bool HideInventory()
    {
        // Check if the player is currently interacting with something
        GameObject current = eventSystem.currentSelectedGameObject;
        if (current != null)
        {
            // Check if it is an interaction button
            string name = current.name;
            if (name.Equals("Use") || name.Equals("Hold") || name.Equals("Cancel") || name.Equals("Held Interaction Button"))
            {
                // Cannot hide the inventory. Wait for the player to finish interacting with the item
                Debug.Log("Should not leave inventory menu");
                inventoryText.SetTemporaryMessage("I should place this item down...");
                return false;
            }
        }

        // Can hide the inventory now
